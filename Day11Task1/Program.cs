...
for (int i = 0; i < galaxies.Count; i++)
{
    for (int j = 0; j < i; j++)
    {
        ...
        var dist = Math.Abs(galaxy2.x - galaxy1.x) + Math.Abs(galaxy2.y - galaxy1.y);
        dist += emptyRows.Count(r => r < Math.Max(galaxy1.y, galaxy2.y) && r > Math.Min(galaxy1.y, galaxy2.y));
        dist += emptyColumns.Count(c => c < Math.Max(galaxy1.x, galaxy2.x) && c > Math.Min(galaxy1.x, galaxy2.x));
        sum += dist;
    }
}
